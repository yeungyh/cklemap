...
from cklemap.sdfs.amps import AMPS
...
class DarcyExp(object):
    ...
    def __init__(self, tpfa, iuobs=None, ssv=None):
        ...
        self.keep = np.flatnonzero(self.cells_neighbors >= 0)
        self.cols = np.concatenate((self.Nc_range, np.tile(self.Nc_range, 4)[self.keep]))
        self.rows = np.concatenate((self.Nc_range, self.cells_neighbors.ravel()[self.keep]))
    ...
    def setup_amps(self, iuobs=None):
        if iuobs is not None:
            self.iuobs = iuobs
            self.amps = AMPS(sps.csc_matrix((np.ones(2 * self.tpfa.Ni + self.Nc), (self.tpfa.rows, self.tpfa.cols)), shape=(self.Nc, self.Nc)),
                sps.csc_matrix((np.ones(self.iuobs.size), (self.iuobs, np.arange(self.iuobs.size))), shape=(self.Nc, self.iuobs.size)),
                self.tpfa.rhs_dirichlet + self.tpfa.rhs_neumann,
                sps.csc_matrix((np.ones(self.Nc + self.keep.size), (self.rows, self.cols)), shape=(self.Nc, self.Nc)))
        else:
            self.iuobs = None
            self.amps = None
    ...
    def residual_sens_Y(self, u, Y):
        # call residual(self, u, Y) before residual_sens_Y(self, u, Y)
        K = np.exp(Y)
        offdiags = (u[self.cells_neighbors] - u[None, :]) * self.tpfa.sens() * K[None, :]
        diags = (self.tpfa.alpha_dirichlet * u - self.tpfa.rhs_dirichlet) * K  - offdiags.sum(axis=0)
        return sps.csc_matrix((np.concatenate((diags, offdiags.ravel()[self.keep])), (self.rows, self.cols)), shape=(self.Nc, self.Nc))
...
#            print(f'update time elasped: {perf_counter() - time_start}')
#            time_start = perf_counter()
            return self.amps.solve_jac(dLdp)
#            print(f'amps time elasped: {perf_counter() - time_start}')
#            return dudp
        else:
