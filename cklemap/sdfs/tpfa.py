...
class TPFA(object):
    ...
    def ops(self, K, q=None):
        ...
        return sps.csc_matrix((np.concatenate((-self.Tgf_interior, -self.Tgf_interior, diag)), (self.rows, self.cols)), shape=(self.Nc, self.Nc)),\
            self.rhs_dirichlet * K + \
            (np.bincount(self.boundary[self.bc.kind == 'N'], q,
             minlength=self.Nc) if q is not None else self.rhs_neumann)

    def sens_old(self):
        Tgf_sens_interior = np.append(
            self.alpha[:2*self.Ni] * np.tile(self.Tgf_interior ** 2, 2) / (self.Thf_interior ** 2), 0.0)
