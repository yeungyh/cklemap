...
def compute_Lreg(geom):
    ...
    return sps.csr_matrix((np.concatenate((-np.ones(Nfint), np.ones(Nfint))), (np.tile(np.arange(Nfint), 2), geom.faces.neighbors[:, :Nfint].ravel())), shape=(Nfint, Nc))
...
class LossVec(object):
    ...
    def __init__(self, Nu, NY, iuobs, uobs, iYobs, Yobs, gamma, L):
        ...
        self.L     = L
        self.grad_u_matrix = sps.csc_matrix((-np.ones(self.iuobs.size), (self.iuobs, np.arange(self.iuobs.size))), shape=(self.Nu, self.iuobs.size))
        self.grad_Y_matrix = sps.vstack([sps.csr_matrix((-np.ones(self.iYobs.size), (np.arange(self.iYobs.size), self.iYobs)), shape=(self.iYobs.size, self.NY)),
                           self.gamma12 * self.L])
